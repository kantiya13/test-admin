...
const createGroupQuestion = async () => {
  ...
  try {
    ...
    questionArray.forEach(question => {
      question.group_id = groupData[0].id
    })
    ...
    if (dataError) {
      console.error('Error inserting order:', dataError);
      return;
    }
  } catch (error) {
    if (error) {
...
<template>
  <div class="h-full">
    <Breadcrumb :topic="'จัดการแบบทดสอบ'" :sub-topic="'เพิ่มข้อมูล'"/>
    <div class="card">
      <div class="m-2">
        <div class="p-4">
          <div class="mt-2">
        ...
        <div class="mt-2">
          <div class="mb-3">
            <div class="row">
              ...
              <div class="col-auto text-end mb-2" v-if="questionArray.length > 1 && index !== 0">
                <div class="action-icon action-icon-delete">
                  <mdi-icon icon="mdiDeleteOutline" @click="removeQuestion(index)" />
                </div>
              </div>
            </div>
            <textarea class="form-control form-control-sm" id="exampleFormControlTextarea1" rows="3" v-model="question.question"></textarea>
          </div>
          <div class="row">
            <div class="col">
              <div>
    ...
    <div class="text-right mt-3">
      <button type="button" class="btn btn-secondary btn-sm mr-1 " @click="cancelCreateQuestion" :disabled="loading">ยกเลิก</button>
      <button type="button" class="btn btn-primary btn-sm ml-1" @click="createGroupQuestion" :disabled="loading">บันทึก</button>
    </div>
  </div>
</template>
